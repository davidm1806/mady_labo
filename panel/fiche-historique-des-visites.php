...
$current_page = $url.'/seca/panel/page_principale.php?menu='.$_GET['menu'];
...
$href_mois = $current_page.'&year='.$annee_selected;
...
$resul_all_visite= mysql_query($query_all);

$visite_by_mounth = mysql_query($query_annee);


while ($visite = mysql_fetch_array($visite_by_mounth)) {
    if($visite['mois']=='01')
...
while ($visite = mysql_fetch_array($resul_all_visite)) {
...
<div class="container-fluid border-top">
    ...
    <div class="row text-muted mt-2">
        <? foreach ($annee_array as $visite){?>
            <div class="col text-center pr-1 font-weight-bold">
                <a href="<?echo $current_page.'&year='.$visite[0] ?>">
                    <div class="text-white <?if($annee_selected==$visite[0]) echo 'bg-success';
                    else echo 'bg-secondary' ?> ">
                        <?echo $visite[0]?><br/>
                        <?echo $visite[1]?> <small>vues</small>
                    </div>
                </a>
            </div>
        <?}?>
    ...
    <div class="row text-muted mt-2">
        ...
        foreach ($mois_array as $visite){?>
            <div class="col-1 text-center pr-1 font-weight-bold">
                <a href="<?echo $href_mois.'&mois='.$visite[2]?>">
                    <div class=" text-white <?if(isset($_GET['mois']) && $_GET['mois']==$visite[2]) echo 'bg-success';
...
<div class="container">
    <table class="table table-bordered table-sm table-striped" id="tabla">
        ...
        <tbody id="myTable">
        <?
        $visite_by_day = mysql_query($query_mois);

        while ($visite = mysql_fetch_array($visite_by_day)) {
           /* if (!empty(geoip_record_by_name('www.google.com'))) {
            ...
            <tr>
                <td><?echo $visite['ip_adress']?></td>
                <td><? echo $visite['pays']?></td>
                <td><?echo $visite['ville']?></td>
                <td><a href="<?echo $visite['url']?>" target="_blank"><?echo $visite['url']?></a></td>
                <td><?echo $visite['nbre_vue']?></td>
                <td><?echo $visite['jour_varchar'].'-'.$visite['mois'].'-'.$visite['annee']?></td>
            </tr>
        <?}?>
        </tbody>
    </table>
